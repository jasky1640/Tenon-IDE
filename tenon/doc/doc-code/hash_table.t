  ...
  function bool insert_item(hash_item item)
    ...
    for i from 1 to 10 do
      if ((hash_table{index, i}.key == -1) and
        ...
        twrite_num(hash_table{index, i}.key);
        twrite_string(hash_table{index, i}.content);
        b_insert = true;
        break;
      end
    end
  ...
  function bool find_item(hash_item item)
    ...
    for i from 1 to 10 do
      if ((hash_table{key_hash, i}.key == item.key) and
        ...
        twrite_num(hash_table{key_hash, i}.key);
        twrite_string(hash_table{key_hash, i}.content);
        b_find = true;
      end
    end
    return b_find;
